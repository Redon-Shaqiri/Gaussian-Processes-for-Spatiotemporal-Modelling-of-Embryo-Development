...
def run_trial(trial_idx, trial_dir, params, seed):
    ...
    random_genes = np.array(genes)[random_indices].tolist()

    multi_output_genes = [gene] + random_genes

    info(
        f"Randomly selected multi-output genes: "
        f"{multi_output_genes}"
    )
    ...
    plot_training_history(training_results["training_losses"], training_results["validation_nlpds"], trial_dir / "training_history_ran.png")

    plot_predictions_vs_true(y_test[:, 0].cpu().numpy(), predictions.mean[:, 0].detach().cpu().numpy(), trial_dir / "predictions_vs_true_ran.png")

    plot_spatial_predictions(spatial_data, trial_dir / "spatial_predictions_ran.png")

    plot_spatial_predictions_uncertainty(spatial_data, trial_dir / "spatial_predictions_uncertainty_ran.png", n_slices=5)

    calibration_results = plot_multi_calibration_curve(model=model, likelihood=likelihood, X_test=X_test, y_test=y_test[:, 0], output_idx=0, output_path=trial_dir / "calibration_curve_ran.png")
