...
from gpembryos.evaluation import all_metrics, get_separable_parameters
from gpembryos.models import SeparableSpatiotemporalGP
...
def run_trial(trial_idx, trial_dir, params, seed):
    ...
    model = SeparableSpatiotemporalGP(inducing_points=inducing_points).double()
    ...
    model_parameters = get_separable_parameters(model, likelihood)
    ...
    plot_training_history(training_results["training_losses"], training_results["validation_nlpds"], trial_dir / "training_history_sep.png")

    plot_predictions_vs_true(y_test.cpu().numpy(), predictions.mean.detach().cpu().numpy(), trial_dir / "predictions_vs_true_sep.png")

    for stage_name in np.unique(stage_test):
        stage = spatial_data[spatial_data["stage"] == stage_name]

        plot_spatial_predictions(stage, trial_dir / f"spatial_predictions_sep_{stage_name}.png")

        plot_spatial_predictions_uncertainty(stage, trial_dir / f"spatial_predictions_uncertainty_sep_{stage_name}.png", n_slices=5)

    calibration_results = plot_calibration_curve(model=model, likelihood=likelihood, X_test=X_test, y_test=y_test, output_path=trial_dir / "calibration_curve_sep.png")
