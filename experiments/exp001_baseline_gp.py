...
from gpembryos.models import ExactGPModel
from gpembryos.preprocessing import gene_expression_values, genes_variability_ranked
from gpembryos.training import exact_gp_training
...
def run_trial(trial_idx, trial_dir, params, seed):
    ...
    # Rank genes in terms of variability
    ...
    # Standardise train and test set
    ...
    model = ExactGPModel(x_t, y_t, likelihood, nu=params["model"]["nu"],).double()
    training_results = exact_gp_training(
        model=model,
    ...
    # Create dataset to compute plots
    ...
    plot_spatial_predictions(spatial_data, trial_dir / "spatial_predictions.png")

    plot_spatial_predictions_uncertainty(spatial_data, trial_dir / "spatial_predictions_uncertainty.png", n_slices=5)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=f"{EXPERIMENT}: Baseline Exact Gaussian Process")
